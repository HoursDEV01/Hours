Hours Core version *0.15.1* is now available from:

  <https://download.Hours.org/Hours-0.15.1/>
...
A number of changes to the way Hours Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 Minutes per kB (0.00001 HOURS). This is to prepare for dropping the minimum transaction fee to 0.1 Minutes per kB (0.0001 HOURS) in the next release.
