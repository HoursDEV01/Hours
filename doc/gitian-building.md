...
*Setup instructions for a Gitian build of Hours Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Hours
...
- [Building Hours Core](#building-Hours-core)
...
Clone the git repositories for Hours and Gitian.
...
in the Hours repository under 'Fetch and create inputs' to install sources which require
...
To build Hours Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Hours repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/Hours/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/Hours-project/Hours
...
For example:
```bash
URL=https://github.com/thrasher-/Hours.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit Hours=${COMMIT} --url Hours=${URL} ../Hours/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit Hours=${COMMIT} --url Hours=${URL} ../Hours/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit Hours=${COMMIT} --url Hours=${URL} ../Hours/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the Hours git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../Hours/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/Hours
SIGPATH=/some/root/path/Hours-detached-sigs

./bin/gbuild --url Hours=${BTCPATH},signature=${SIGPATH} ../Hours/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/Hours-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/Hours-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/Hours-osx-build.assert
```
...
[Hours-project/gitian.sigs.ltc](https://github.com/Hours-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
