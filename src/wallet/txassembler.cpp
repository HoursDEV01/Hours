    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional HRS outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes HRS and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::HRS_TO_HRS) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a HRS-to-HRS transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::HRS_ONLY;
        ...
        // If HRS-to-HRS fails, create a peg-out transaction
